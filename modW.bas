...
Private Declare Function SysAllocStringLen _
                Lib "oleaut32" (ByVal OleStr As Long, _
...
Private Declare Function TextOut _
                Lib "gdi32" _
                Alias "TextOutW" (ByVal hDC As Long, _
                                  ByVal X As Long, _
                                  ByVal Y As Long, _
...
Private Declare Function SetClipboardData _
                Lib "user32" (ByVal Format As Long, _
...
Private Declare Function GlobalAlloc _
                Lib "kernel32" (ByVal Flags As Long, _
...
Private Declare Sub RtlMoveMemory _
                Lib "kernel32" (ByVal pDest As Long, _
...
Public Function TextWPrint&(ToObject As Object, _
                            TextW$, _
                            Optional PercentMargin% = 2, _
                            Optional RightToLeft, _
    ...
    With ToObject
        ...
        For l& = 0 To n&
            ...
            If PT.X + (XP& * 2) > W& Then 'Text to long to fit the width.
                ...
                TextOut .hDC, X&, Y, (StrPtr(s$)), (Len(s$)): s$ = "": Y& = Y& + h&
            End If
...
ErrTextWPrint:
    MsgBox "Error number " & Err & " in function TextWPrint():" & vbLf & vbLf & Err.Description, vbCritical: Err.Clear
End Function

Public Function TextW2HTML(TextW$, _
                           Optional ToFile$, _
                           Optional OpenFile As Boolean, _
                           Optional ByVal Title$, _
                           Optional ByVal FontName$ = "Tahoma", _
                           Optional ByVal FontSize% = 3, _
                           Optional Right2Left As Boolean) As String
    ...
    TextW2HTML = "<html" & TextW2HTML & "><head><meta http-equiv=""Content-Type"" " & "content=""text/html; charset=UTF-8""><title>" & Title$ & "</title></head><body>" & vbNewLine & "<p><font face=""" & FontName$ & """ size=""" & FontSize% & """>" & vbNewLine & Join(Split(TextW$, vbNewLine), "<br>" & vbNewLine) & vbNewLine & "</font></p></body></html>"
...
ErrTextW2HTML:
    MsgBox "Error number " & Err & " in function TextW2HTML():" & vbLf & vbLf & Err.Description, vbCritical: Err.Clear
End Function

Public Function MsgBoxW(ByVal Prompt$, _
                        Optional ByVal Buttons As VbMsgBoxStyle = vbInformation, _
                        Optional ByVal Title$, _
                        Optional ByVal hWndOwner&) As VbMsgBoxResult
...
Public Function GetRndChrs$(ByVal Table As TablesUTF8, _
                            Optional NoOffChars% = 1, _
                            Optional IncludeInfo As Boolean)

    If NoOffChars% < 1 Then Exit Function
    If Not TablesU% Then InitModuleW
    If Table > TablesU% Or Table < 0 Then MsgBoxW MwErrTable, 48: Exit Function
...
Function ShowCharCodeInfo(CharCode&) As Boolean 'Show info of the charcode in a messagebox.
    ...
    If CharCode& > MwMaxUTF8 Or CharCode& < 0 Then MsgBoxW "Invalid charcode, " & LCase(C) & "0 - " & MwMaxUTF8 & ".", 48: Exit Function
    ...
    For X& = 0 To TablesU%
        ...
        With UTF8(X&)
            ...
            If CharCode& >= .From And CharCode& <= .To Then
                ShowCharCodeInfo = True: If CharCode& = 0 Then s$ = "Null" Else s$ = ChrW(CharCode&)
                MsgBoxW "The charcode " & CharCode& & " '" & s$ & "' belongs to " & .Name & "." & vbLf & vbLf & C & .From & " - " & .To & ".": Exit Function
            End If
...
Public Function CreateCharTable$(ByVal Table As TablesUTF8, _
                                 Optional SaveAndShowInNotepad As Boolean, _
                                 Optional ReplaceFileIfExist As Boolean, _
                                 Optional RetFileName$)
    ...
    CreateCharTable$ = vbNewLine & String(50, "*") & vbNewLine & RetFileName$ & " (" & T& - f& + 1 & " characters)" & vbNewLine & String(50, "*") & vbNewLine & vbNewLine & Join(s$, vbNewLine)
...
Function FindTable(ByVal SerchString$, Optional ShowInfo As Boolean) As UTF8Table()
    ...
    For X& = 0 To TablesU%
        ...
        With UTF8(X&)
            ...
            If InStr(1, LCase(.Name), SerchString$) Then
                ...
                s$ = s$ & "Index = " & X& & vbTab & Format(.From, Mw5x0) & " - " & Format(.To, Mw5x0) & ", " & .Name & vbNewLine: n& = n& + 1
            End If
    ...
    If ShowInfo Then 'Show info if selected
        If n& Then
            s$ = "Did find " & n& & " table" & IIf(n& > 1, "s", "") & ":" & vbLf & vbLf & s$ & vbLf & vbLf & "Do you want to create " & IIf(n& > 1, "them?", "it?"): X& = 292
...
Public Function GetLocalLanguageCountry$(Optional bGetCountry As Boolean, _
                                         Optional bNativeName As Boolean)
...
Public Sub Contact(Optional eMail$, _
                   Optional Name$, _
                   Optional Subject$, _
                   Optional BodyText$, _
                   Optional AttachedFile$, _
                   Optional CC$, _
                   Optional BCC$)
    ...
    If Subject$ = vbNullString Then Subject$ = App.Title & " " & App.Major & "." & App.Minor & "." & App.Revision
    s$ = "mailto:" & Name$ & " <" & eMail$ & ">"
    s$ = s$ & "?subject=" & Subject$

    If LenB(BodyText$) Then s$ = s$ & "&Body=" & Replace(BodyText$, vbNewLine, "%0D%0A")
    ...
    If LenB(AttachedFile$) Then
        If Dir(AttachedFile$) = vbNullString Then 'Can not find attached file...
            If MsgBoxW("Cant find the attached file '" & AttachedFile$ & "'." & vbLf & vbLf & "Do you want to continue?", 308) = vbNo Then Exit Sub
...
Public Function TextWOpenInNotepad(TextW$, _
                                   Optional ByVal SaveToFile$, _
                                   Optional UsASCII As Boolean) As Boolean
...
Public Function TextWLoad$(ReadFromFile$, _
                           Optional UsASCII As Boolean, _
                           Optional OpenInNotepad As Boolean)
...
Private Function SaveOpenFile(TextW$, _
                              Optional ByVal sFile$, _
                              Optional OpenInNotepad As Boolean, _
                              Optional ReadFromFile As Boolean, _
                              Optional RetText$, _
                              Optional AsASCII As Boolean) As Boolean
...
ErrSaveOpenFile:
    MsgBox "Error number " & Err & " in function SaveOpenFile():" & vbLf & sFile$ & vbLf & vbLf & Err.Description, vbCritical: Resume ExitSaveOpenFile
End Function
