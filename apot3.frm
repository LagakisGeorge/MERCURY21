...
Begin VB.Form apot3 
   ...
   Begin VB.CommandButton Command3 
      Caption         =   "Ενημέρωση με τελ.ημερ.αγοράς"
      Height          =   495
      Left            =   7560
      TabIndex        =   28
      Top             =   6000
...
Attribute FIsHidden.VB_VarUserMemId = 1073938433

Dim F_COL As TrueOleDBGrid80.Column
Attribute F_COL.VB_VarUserMemId = 1073938435

Dim F_epan As Integer
Attribute F_epan.VB_VarUserMemId = 1073938436
...
Attribute F_COLS.VB_VarUserMemId = 1073938441
...
Attribute fList1ARX.VB_VarUserMemId = 1073938443

Dim F_LHM1 As String
Attribute F_LHM1.VB_VarUserMemId = 1073938444
Dim F_LHM2 As String
Attribute F_LHM2.VB_VarUserMemId = 1073938445
Dim f_lHM3 As String
Attribute f_lHM3.VB_VarUserMemId = 1073938446

Dim f_lab1 As String
Attribute f_lab1.VB_VarUserMemId = 1073938447
Dim f_lab2 As String
Attribute f_lab2.VB_VarUserMemId = 1073938448
Dim f_lab3 As String
Attribute f_lab3.VB_VarUserMemId = 1073938449
Dim f_lab4 As String
Attribute f_lab4.VB_VarUserMemId = 1073938450
Dim f_lab5 As String
Attribute f_lab5.VB_VarUserMemId = 1073938451
Dim f_lab6 As String
Attribute f_lab6.VB_VarUserMemId = 1073938452

Dim f_num1 As String
Attribute f_num1.VB_VarUserMemId = 1073938453
Dim f_num2 As String
Attribute f_num2.VB_VarUserMemId = 1073938454
Dim f_num3 As String
Attribute f_num3.VB_VarUserMemId = 1073938455
...
Private Sub aritmos_pelaton_Click()
    ...
    R.Open UCase("select pol,eidos,ajia_apou,SHM1 from parastat"), Gdb, adOpenDynamic, adLockOptimistic


    pol = ""
    R.MoveFirst
    Do While Not R.EOF
        If Not IsNull(R("eidos") + R("pol") + R("SHM1")) Then
            If R("pol") = "1" Then     ' παροχη And Left(r("SHM1"), 1) = "2" Then
                pol = pol + "'" + R("eidos") + "',"
            End If
        End If
        R.MoveNext
    ...
    X = " select PARASTAT.TITLOS,count(*) AS [ΔΙΑΦ.ΕΙΔΗ],count(DISTINCT ATIM) AS [ΠΕΛΑΤΕΣ] "
    X = X + " FROM EGGTIM INNER JOIN PARASTAT ON LEFT(ATIM,1)=PARASTAT.EIDOS "
    X = X + " WHERE LEFT(ATIM,1) IN (" + pol + ") and "
    X = X + " HME>='" + Format(apo, "MM/DD/YYYY") + "' AND HME < '" + Format(DateAdd("D", 1, eos), "MM/DD/YYYY") + "'"
    X = X + " GROUP BY PARASTAT.TITLOS"
    ...
    If Check1 Then
        'typos2
        On Error GoTo 0
        print3_xar UCase(X), "0111111111", "", 0
    End If
...
' ΑΠΟ ΤΟ ΕΤΟΙΜΟ ΡΕΠΟΡΤ ΠΟΥ ΕΧΩ ΕΠΙΛΕΞΕΙ
    ...
    For KK = 0 To List1.ListCount - 1
        If List1.Selected(KK) Then
            ...
            temp = Val(Right(List1.List(KK), 2))
            X = X + f_list1(temp, 0)
            mc = mc + 1
            'x = x + f_list1(Val(Right(List1.List(KK), 2)), 0)
            F_sum(mc) = f_list1(temp, 1)
            F_ALIG(mc) = f_list1(temp, 2)
            F_mWidth(mc) = f_list1(temp, 3)
            If f_list1(temp, 1) = 1 Then
                SUgm_str = SUgm_str + "1"
            Else
                SUgm_str = SUgm_str + "0"
            End If
    ...
    If Kinhseis Then
        ...
        a = "SELECT KODE,SUM( CASE APOT  WHEN 1  THEN XRE ELSE 0  END ) AS S1X," _
          ...
          & " INTO DOKEGGT1 "
        a = a + " FROM EGGTIM WHERE (NOT APOT IS NULL) AND (NOT EKPT IS NULL) AND (NOT XRE IS NULL) AND (NOT PIS IS NULL) AND (NOT TIMM IS NULL) AND " _
...
'======== ΜΕΣΗ ΤΙΜΗ αγοράς ΣΕ ΗΜΕΡΟΜΗΝΙΑΚΟ ΔΙΑΣΤΗΜΑ ===========================================================================
    ...
    X = "SELECT KODE,ONOMA,SUM(POSO) as [ΣΥΝ.ΠΟΣΟΤ],SUM(POSO*TIMM*(1-EKPT/100)) AS  [ΣΥΝ.ΑΞΙΑΣ],(CASE WHEN SUM(POSO)=0 THEN 0 ELSE SUM(POSO*TIMM*(1-EKPT/100))/SUM(POSO) END ) AS [MEΣ.ΤΙΜΗ ΑΓΟΡΑΣ] FROM EGGTIM WHERE LEFT(ATIM,1) IN ('G','g','λ') AND HME>='" + Format(apo, "MM/DD/YYYY") + "' AND HME < '" + Format(DateAdd("D", 1, eos), "MM/DD/YYYY") + "' group by KODE,ONOMA"

    EGGTIM.Open X, Gdb, adOpenDynamic, adLockOptimistic
    Gdb.Execute "SELECT KODE,(CASE WHEN SUM(POSO)=0 THEN 0 ELSE SUM(POSO*TIMM*(1-EKPT/100))/SUM(POSO) END ) AS MESXTI  INTO DOKMESXTI FROM EGGTIM WHERE LEFT(ATIM,1) IN ('G','g','λ') AND HME>='" + Format(apo, "MM/DD/YYYY") + "' AND HME < '" + Format(DateAdd("D", 1, eos), "MM/DD/YYYY") + "' group by KODE,ONOMA"
    Gdb.Execute "UPDATE EID SET EID.MESXTI=DOKMESXTI.MESXTI FROM DOKMESXTI LEFT JOIN EID ON EID.KOD=DOKMESXTI.KODE  "
    If Check1 Then
        'typos2
        On Error GoTo 0
        SUgm_str = "0111111111111111"
        print3_xar UCase(X), SUgm_str, Format(apo, "DD/MM/YYYY") + " ΕΩΣ " + Format(eos, "DD/MM/YYYY"), 0        ' RR.RecordSource"
    End If
...
On Error Resume Next
 ...
 If Len(f_PALIAXRONIA) > 0 Then
      FGDB.Open gConnect + ";DATABASE=" + f_PALIAXRONIA
      TelProm
      FGDB.Close
...
'======== ΜΕΣΗ ΤΙΜΗ ΠΩΛΗΣΗΣ ΣΕ ΗΜΕΡΟΜΗΝΙΑΚΟ ΔΙΑΣΤΗΜΑ ===========================================================================
    ...
    X = "SELECT KODE,ONOMA,SUM(POSO) as [ΣΥΝ.ΠΟΣΟΤ],SUM(POSO*TIMM*(1-EKPT/100)) AS  [ΣΥΝ.ΑΞΙΑΣ],SUM(POSO*TIMM*(1-EKPT/100))/SUM(POSO) AS [MEΣΗ ΤΙΜΗ ΠΩΛΗΣΗΣ] FROM EGGTIM WHERE LEFT(ATIM,1) IN ('T','t','L') AND HME>='" + Format(apo, "MM/DD/YYYY") + "' AND HME < '" + Format(DateAdd("D", 1, eos), "MM/DD/YYYY") + "' group by KODE,ONOMA"
...
Private Sub Form_Resize()
    mForm_Resize Me, 14040, 9915, fh, fw, ft, fl
...
'ΕΚΤΥΠΩΣΗ ΣΤΟ ΣΥΝΟΛΟ ΤΩΝ ΑΠΟΘΗΚΩΝ
    ...
    '
    a = "SELECT LEFT(KOD,14)AS [ΚΩΔ],LEFT(EID.PROM,10) AS [ΠΡΟΜΗΘ],(SELECT PERIGRAFH FROM PINAKES WHERE TYPOS=11 AND AYJON=EID.AEG) AS [ΟΙΚΟΓ] ,"

    a = a + "(SELECT TOP 1 PERIGRAFH FROM PINAKES WHERE TYPOS=3 AND TIMH=EID.AEG AND CAST(KODLOG AS INT)=AYJON ) AS [ΥΠΟ-ΟΙΚ],LEFT(EID.ONO,35) AS [ΠΕΡΙΓΡΑΦΗ],"


    a = a + "S0X AS [ΑΠΟΓΡ], S1X AS [ΑΓΟΡΕΣ],S1P AS [ΠΩΛΗΣΕΙΣ],S0X+S1X-S1P AS [ΥΠΟΛ], "
    a = a + "AS1X AS [ΑΞ.ΑΓΟΡ],AS1P AS [ΑΞ.ΠΩΛ] "
    ...
    If ANS2 = vbYes Then
        synt = " WHERE [ΑΓΟΡΕΣ]>0 OR [ΠΩΛΗΣΕΙΣ]>0  OR [ΑΞ.ΠΩΛ]>0"
    ...
    ANS2 = MsgBox("ΣΥΝΟΛΑ ΑΝΑ ΟΙΚΟΓΕΝΕΙΑ;", vbYesNo)
    If ANS2 = vbYes Then

        X = "SELECT  [ΟΙΚΟΓ],[ΥΠΟ-ΟΙΚ],SUM([ΑΓΟΡΕΣ]) AS [ΑΓΟΡΕΣ],SUM([ΠΩΛΗΣΕΙΣ]) AS [ΠΩΛΗΣΕΙΣ], SUM([ΑΞ.ΑΓΟΡ]) AS [ΑΞ.ΑΓΟΡΩΝ],SUM([ΑΞ.ΠΩΛ]) AS [ΑΞ.ΠΩΛΗΣ]  FROM DOKEGGT3 " + synt + " GROUP BY  [ΟΙΚΟΓ],[ΥΠΟ-ΟΙΚ]  ORDER BY [ΟΙΚΟΓ],[ΥΠΟ-ΟΙΚ]"
    Else
        X = "SELECT * FROM DOKEGGT3 " + synt + " ORDER BY [ΟΙΚΟΓ],[ΥΠΟ-ΟΙΚ]"
    End If
...
Private Sub Command5_Click()
    ...
    If Not FIsHidden Then
        With KRITHRIA.Grid1
            ...
            N = N + 1: .TextMatrix(N, 0) = "Περιγραφή 2η": .TextMatrix(N, 3) = "EID.ONO": .TextMatrix(N, 4) = "c"
...
'ΕΚΤΥΠΩΣΗ ΚΑΤΑ ΑΠΟΘΗΚΗ
    ...
    If Check1 Then
        'typos2
        print3_xar UCase(X), "00001111111111111111", "", 0
    End If
...
'ΕΚΤΥΠΩΣΗ ΣΤΟ ΣΥΝΟΛΟ ΤΩΝ ΑΠΟΘΗΚΩΝ
    ...
    a = "SELECT KODE," _
      & " SUM( CASE   WHEN LEFT(ATIM,1)='λ'  THEN XRE ELSE 0  END ) AS S0X," _
      & " SUM( CASE   WHEN LEFT(ATIM,1) IN ('G','g')  THEN POSO ELSE 0  END ) AS S1X," _
      & " SUM( CASE WHEN PIS IS NULL  OR LEFT(ATIM,1) NOT IN (" + polhs + ")  THEN 0 ELSE POSO END) AS S1P," _
      & " SUM( CASE WHEN XRE*TIMM IS NULL THEN 0 ELSE XRE*TIMM*(1- (CASE WHEN EKPT IS NULL THEN 0 ELSE EKPT END)/100) END  ) AS AS1X," _
      & " SUM( CASE WHEN POSO*TIMM IS NULL OR LEFT(ATIM,1) NOT IN (" + polhs + ")  THEN 0 ELSE POSO*TIMM*(1-(CASE WHEN EKPT IS NULL THEN 0 ELSE EKPT END)/100) END ) AS AS1P, " _
      & " SUM( CASE WHEN KERDOS IS NULL OR LEFT(ATIM,1) NOT IN (" + polhs + ")  THEN 0 ELSE KERDOS END ) AS KERD1 INTO DOKEGGT1 "
    a = a + " FROM EGGTIM WHERE  left(ATIM,1) NOT IN ('σ','Σ') AND  " _
...
Function Get3_AJ(ByRef pol As String, polepis As String, ago As String, AGOEPIS As String, ByRef PolXParoxh As String, Paroxh As String, ByRef PolAJ As String, AgoAJ As String) As Boolean
    ...
    PolAJ = Left(PolAJ, Len(PolAJ) - 1)
    AgoAJ = Left(AgoAJ, Len(AgoAJ) - 1)
...
'SELECT EID.KOD,(SELECT TOP 1 TIMM*(100-EKPT)/100 FROM EGGTIM WHERE LEFT(ATIM,1) IN('G','Ξ') AND KODE=EID.KOD ORDER BY HME DESC) AS TIMHAG  INTO DOKTIMHAG  FROM EID
    ...
    Dim polepis As String, pol As String, ago As String, APIS
    Dim a
    a = Get3_AJ(polhs, polepis, ago, AGOEPIS, PolXParoxh, Paroxh, PolAJ, AgoAJ)
    ...
    Gdb.Execute "UPDATE TIM SET KERDOS=0 where HME>='" + Format(apo, "MM/DD/YYYY") + "' AND HME < '" + Format(DateAdd("D", 1, eos), "MM/DD/YYYY") + "'"

    Gdb.Execute "UPDATE EGGTIM SET KERDOS=0 where HME>='" + Format(apo, "MM/DD/YYYY") + "' AND HME < '" + Format(DateAdd("D", 1, eos), "MM/DD/YYYY") + "'"




    Dim EGGTIM As New ADODB.Recordset
    EGGTIM.Open "SELECT * FROM EGGTIM WHERE LEFT(ATIM,1) IN (" + PolAJ + ") AND HME>='" + Format(apo, "MM/DD/YYYY") + "' AND HME < '" + Format(DateAdd("D", 1, eos), "MM/DD/YYYY") + "' ORDER BY HME", Gdb, adOpenDynamic, adLockOptimistic

    Dim R As New ADODB.Recordset
    Dim EID As New ADODB.Recordset

AGNOSTOI.AddItem "ΔΕΝ ΒΡΕΘΗΚΑΝ ΟΙ ΚΩΔΙΚΟΙ:"
    ...
    Do While Not EGGTIM.EOF
        ...
        d = EGGTIM("HME")
        k = 0
        R.Open "SELECT top 1 * FROM EGGTIM WHERE KODE='" + a + "' AND LEFT(ATIM,1) IN ('G','Ξ','g','λ') AND HME<='" + Format(d, "MM/DD/YYYY") + "' ORDER BY HME DESC", Gdb, adOpenDynamic, adLockOptimistic
        If R.EOF Then
            'PAIRNO THN TIMH APO TO EID
            EID.Open "SELECT XTI FROM EID WHERE KOD='" + a + "'", Gdb, adOpenForwardOnly, adLockReadOnly
            If EID.EOF Then
                k = MsgBox("ΔΕΝ ΥΠΑΡΧΕΙ ΣΤΑ ΕΙΔΗ Ο ΚΩΔΙΚΟΣ " + a + Chr(13) + "Συνέχεια Ν/Ο", vbYesNo)
                If k = vbNo Then
                    Exit Sub
                End If
            Else
        ...
        MATIM = EGGTIM("atim")
        mHME = Format(EGGTIM("hme"), "MM/DD/YYYY")
        MM = EGGTIM("kerdos")
        EGGTIM.Update
        EGGTIM.MoveNext
        If IsNull(MM) Then MM = 0
        Gdb.Execute "UPDATE TIM SET KERDOS=KERDOS + " + Str(MM) + " WHERE HME='" + mHME + "' AND ATIM='" + MATIM + "'"
    Loop
    EGGTIM.Close
...
'===================================================================================
    ...
    Dim EGGTIM As New ADODB.Recordset
    ''E','α',
    EGGTIM.Open "SELECT * FROM EGGTIM WHERE LEFT(ATIM,1) IN ('E','α','λ'," + ago + ") AND HME>='" + Format(apo, "MM/DD/YYYY") + "' AND HME < '" + Format(DateAdd("D", 1, eos), "MM/DD/YYYY") + "' ORDER BY HME", Gdb, adOpenDynamic, adLockOptimistic
    ...
    Do While Not EGGTIM.EOF
        ...
        Atim = EGGTIM("atim")
        k = k + 1
        If Not IsNull(EGGTIM("kode")) Then
            If IsNull(EGGTIM("TIMM")) Then
            Else
              If IsNull(EGGTIM("EKPT")) Then
              Else
                  If EGGTIM("TIMM") > 0 Then
                     Gdb.Execute "UPDATE EID SET XTI=" + Str(nNull(EGGTIM("TIMM") * (1 - EGGTIM("EKPT") / 100))) + ",CH1='" + Atim + "',HPAR='" + Format(d, "MM/DD/YYYY") + "',PAR=" + Str(EGGTIM("POSO")) + "  where KOD='" + a + "'", N
                  Else
                     Gdb.Execute "UPDATE EID SET CH1='" + Atim + "',HPAR='" + Format(d, "MM/DD/YYYY") + "',PAR=" + Str(EGGTIM("POSO")) + "  where KOD='" + a + "'", N
                  End If
              End If
        ...
        If N > 0 Then
            MDIForm1.Caption = Format(k, "####") + " - " + Format(d, "dd/mm/yyyy hh:mm") + " "
        End If
...
'===================================================================================
    ...
    EGGTIM.Open " select PEL.EPO,EGGTIM.* FROM EGGTIM INNER JOIN PEL ON EGGTIM.EIDOS=PEL.EIDOS AND PEL.KOD=PELKOD         WHERE LEFT(ATIM,1) IN ('G','g') order by HME", FGDB, adOpenDynamic, adLockOptimistic
    ...
    Do While Not EGGTIM.EOF
        ...
        k = k + 1
        If Not IsNull(EGGTIM("kode")) Then
            Gdb.Execute "UPDATE EID SET PROM='" + Left(EGGTIM("EPO"), 30) + "'  where KOD='" + a + "'", N
        End If
...
Private Sub Form_Load()
    ...
    Dim R As Integer
    R = 0 + 0: f_list1(R, 0) = ",LEFT(MON,5) AS [Mον.Μετ]": f_list1(R, 1) = 0: f_list1(R, 2) = 1: f_list1(R, 3) = 3000: f_list1(R, 4) = "Μον.Μέτρ."
    ...
    R = R + 1: f_list1(R, 0) = ",POS AS [Υπόλοιπο]": f_list1(R, 1) = 1: f_list1(R, 2) = 1: f_list1(R, 3) = 1600: f_list1(R, 4) = "Υπόλοιπο"
    ...
    R = R + 1: f_list1(R, 0) = ",CONVERT(CHAR(8),HPAR,3)  as [Ημ.Τελ.Αγοράς]": f_list1(R, 1) = 0: f_list1(R, 2) = 1: f_list1(R, 3) = 1600: f_list1(R, 4) = "Ημερ.Τελ.Αγοράς"
    ...
    R = R + 1: f_list1(R, 0) = ",(CASE AS1P WHEN NULL THEN 0 ELSE AS1P END)  AS [Αξ1.Αγ.Περιόδ]": f_list1(R, 1) = 1: f_list1(R, 2) = 1: f_list1(R, 3) = 1600: f_list1(R, 4) = "Απ1.Αξ.Εξ.Περιόδου"
    R = R + 1: f_list1(R, 0) = ",POS01 as [Υπ.Απ1]": f_list1(R, 1) = 1: f_list1(R, 2) = 1: f_list1(R, 3) = 1300: f_list1(R, 4) = "Υπολ.Απ1"
    ...
    R = R + 1: f_list1(R, 0) = ",XTI*POS03 AS [Αξ.Απογρ.απ.3]": f_list1(R, 1) = 1: f_list1(R, 2) = 1: f_list1(R, 3) = 1600: f_list1(R, 4) = "Αξία Απογραφής απ3"
    ...
    R = R + 1: f_list1(R, 0) = ",XTI*POS04 AS [Αξ.Απογρ.απ4]": f_list1(R, 1) = 1: f_list1(R, 2) = 1: f_list1(R, 3) = 1600: f_list1(R, 4) = "Αξία Απογραφής απ4"
    ...
    R = R + 1: f_list1(R, 0) = ", ONO2 AS [Περιγραφή 2η]": f_list1(R, 1) = 0: f_list1(R, 2) = 1: f_list1(R, 3) = 1600: f_list1(R, 4) = "Περιγραφή 2η"
    ...
    For k = 0 To UBound(f_list1)
        If Len(f_list1(k, 4)) > 0 Then
            List1.AddItem f_list1(k, 4) + Space(40) + Format(k, "00")
        End If
    Next

    List1.ToolTipText = "Με click επιλέγετε το πεδία" + " Με σύρσιμο μπορείτε να αλλάξετε την σειρά"
...
Private Sub GR2_DblClick()
    ...
    ' kartella
    GR2.Col = 0
    apot21.Text1.Text = GR2.Text

    GR2.Col = 1

    apot21.Text2.Text = GR2.Text
...
Private Sub List1_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If ARX = TEL Then  'ΠΑΡΕΜΕΙΝΕ ΣΤΟ ΙΔΙΟ ΚΕΛΙ
        'ΑΣΤΟ
    Else

        If TEL > ARX Then
            S = List1.List(ARX)
            List1.RemoveItem ARX

            List1.AddItem S, TEL
            List1.Selected(TEL) = True


        Else
            List1.AddItem List1.List(ARX), TEL
            List1.Selected(TEL) = True
            List1.RemoveItem ARX + 1
        End If
...
'B = "CREATE VIEW dbo.[EIDT3]" _
   ...
   Dim a As String
    ...
    Gdb.Execute "UPDATE EID SET POS01=0,POS02=0,POS03=0,POS04=0,POS=0", d
...
'===================================================================================
    ...
    Dim EGGTIM As New ADODB.Recordset
    EGGTIM.Open "SELECT * FROM EGGTIM WHERE LEFT(ATIM,1) IN ('l','L') AND HME>='" + Format(apo, "MM/DD/YYYY") + "' AND HME < '" + Format(DateAdd("D", 1, eos), "MM/DD/YYYY") + "' ORDER BY HME", Gdb, adOpenDynamic, adLockOptimistic
    ...
    Do While Not EGGTIM.EOF
        ...
        k = k + 1
        If Not IsNull(EGGTIM("kode")) Then
            Gdb.Execute "UPDATE EID SET HM3='" + Format(d, "MM/DD/YYYY") + "' where KOD='" + a + "'", N
        End If
        If N > 0 Then
            MDIForm1.Caption = Format(k, "####") + " - " + Format(d, "dd/mm/yyyy hh:mm") + " "
        End If
    ...
    On Error Resume Next
    Gdb.Execute "DROP TABLE EIDFOTOS"

    Gdb.Execute "SELECT * INTO EIDFOTOS FROM EID WHERE POS<=0 AND HM3<'" + Format(DateAdd("D", -90, Now), "MM/DD/YYYY") + "'"
