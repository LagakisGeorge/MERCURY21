...
Begin VB.Form apot2vmn 
   ...
   Begin VB.PictureBox Picture2 
      ...
      Begin VB.ComboBox ftp 
         Height          =   300
         ItemData        =   "apot2vm.frx":0086
         Left            =   13470
         List            =   "apot2vm.frx":0090
      ...
      Begin VB.ComboBox Combo2 
         Height          =   300
         ItemData        =   "apot2vm.frx":00A3
         Left            =   2280
         List            =   "apot2vm.frx":00A5
         Style           =   2  'Dropdown List
         TabIndex        =   23
         Top             =   3600
         Width           =   1575
...
Private Sub Command4_Click()
    ...
    a = InputBox("ΑΡΙΘΜΟΣ ΤΙΜΟΛ.ΑΓΟΡΑΣ (ME * ΕΠΙΛΕΓΩ ΗΜΕΡΟΜΗΝΙΕΣ  ME % ΕΠΙΛΕΓΩ ΑΡΧΙΚΑ ΚΩΔΙΚΟΥ ME $ ΑΡΧΙΚΑ ΠΡΟΜΗΘΕΥΤΗ")
    ...
    If a = "*" Then
      ...
      ' a = InputBox("ΑΡΙΘΜΟΣ ΤΙΜΟΛ.ΑΓΟΡΑΣ (ME * ΕΠΙΛΕΓΩ ΗΜΕΡΟΜΗΝΙΕΣ  ME % ΕΠΙΛΕΓΩ ΑΡΧΙΚΑ ΚΩΔΙΚΟΥ ")
       
       Dim r2 As New ADODB.Recordset
       'ΝΑ ΜΗΝ ΕΜΦΑΝΙΖΕΙ ΑΥΤΑ ΠΟΥ ΕΧΟΥΝ 0 ΥΠΟΛΟΙΠΟ Ή ΑΥΤΑ ΠΟΥ ΕΧΟΥΝ ΗΔΗ ΦΩΤΟΓΡΑΦΙΕΣ (ΓΙΑΤΙ ΕΙΝΑΙ ΗΔΗ ΑΝΑΡΤΗΜΕΝΑ ΣΤΟ SITE)
       r2.Open "SELECT DISTINCT KODE FROM EGGTIM G inner JOIN EID E ON G.KODE=E.KOD WHERE   LEFT(ATIM,1) IN ('G','g') AND (E.NUM3 is null or  E.NUM3='2')  AND E.POS01+E.POS02+E.POS03+E.POS04>0 AND (LEN(E.PIC0)<2  OR E.PIC0 IS NULL) AND  HME>='" + Format(CDate(B), "MM/DD/YYYY") + "' ORDER BY KODE", Gdb, adOpenDynamic, adLockOptimistic
       ...
       EGGTIM2.RecordSource = "SELECT DISTINCT KODE FROM EGGTIM G inner JOIN EID E ON G.KODE=E.KOD WHERE   LEFT(ATIM,1) IN ('G','g') AND E.NUM3<>'2' AND E.POS01+E.POS02+E.POS03+E.POS04>0 AND (LEN(E.PIC0)<2  OR E.PIC0 IS NULL)  AND  HME>='" + Format(CDate(B), "MM/DD/YYYY") + "' ORDER BY KODE"
      ...
      '============================================================
       ...
       B = InputBox("ΑΡΧΙΚΑ ΚΩΔΙΚΟΥ  ", , "")
       
       
       
       Dim R28 As New ADODB.Recordset
       'ΝΑ ΜΗΝ ΕΜΦΑΝΙΖΕΙ ΑΥΤΑ ΠΟΥ ΕΧΟΥΝ 0 ΥΠΟΛΟΙΠΟ Ή ΑΥΤΑ ΠΟΥ ΕΧΟΥΝ ΗΔΗ ΦΩΤΟΓΡΑΦΙΕΣ (ΓΙΑΤΙ ΕΙΝΑΙ ΗΔΗ ΑΝΑΡΤΗΜΕΝΑ ΣΤΟ SITE)
       R28.Open "SELECT DISTINCT KODE FROM EGGTIM G inner JOIN EID E ON G.KODE=E.KOD WHERE KODE LIKE '" + B + "%' and   LEFT(ATIM,1) IN ('G','g') AND (E.NUM3 is null or  E.NUM3='2')  AND E.POS01+E.POS02+E.POS03+E.POS04 >0 AND (LEN(E.PIC0)<2  OR E.PIC0 IS NULL)   ORDER BY KODE", Gdb, adOpenDynamic, adLockOptimistic
       
       ' r2.Open "SELECT DISTINCT KODE FROM EGGTIM inner JOIN EID ON EGGTIM.KODE=EID.KOD WHERE   LEFT(ATIM,1) IN ('G','g') AND EID.NUM3<>2 AND  KODE NOT IN (SELECT KOD FROM EID WHERE POS01+POS03<=0 OR LEN(PIC0)>2) AND  HME>='" + Format(CDate(b), "MM/DD/YYYY") + "' ORDER BY KODE", Gdb, adOpenDynamic, adLockOptimistic
       
       
       Open "C:\MERCVB\NEW_PRODUCTS.TXT" For Output As #1
       Do While Not R28.EOF
              Print #1, R28!KODE
              R28.MoveNext
       Loop
       Close #1
       ...
       'EGGTIM2.RecordSource = "SELECT DISTINCT KODE FROM EGGTIM WHERE   LEFT(ATIM,1) IN ('G','g') AND  KODE NOT IN (SELECT KOD FROM EID WHERE POS01+POS03<=0 OR LEN(PIC0)>2) AND  HME>='" + Format(CDate(b), "MM/DD/YYYY") + "' ORDER BY KODE" '
       EGGTIM2.RecordSource = "SELECT DISTINCT KODE FROM EGGTIM G inner JOIN EID E ON G.KODE=E.KOD WHERE  KODE LIKE '" + B + "%' and    LEFT(ATIM,1) IN ('G','g') AND E.NUM3<>'2' AND E.POS01+E.POS02+E.POS03+E.POS04 >0 AND (LEN(E.PIC0)<2  OR E.PIC0 IS NULL)  ORDER BY KODE"
       '"SELECT DISTINCT KODE FROM EGGTIM WHERE  LEFT(ATIM,1) IN ('G','g')  AND KODE NOT IN (SELECT KOD FROM EID WHERE LEN(PIC0)>2) AND  HME>='" + Format(CDate(b), "MM/DD/YYYY") + "' ORDER BY KODE"
       EGGTIM2.Refresh
       ...
       B = InputBox("ΑΡΧΙΚΑ ΠΡΟΜΗΘΕΥΤΗ  ", , "")
       
       
       
       Dim R29 As New ADODB.Recordset
       'ΝΑ ΜΗΝ ΕΜΦΑΝΙΖΕΙ ΑΥΤΑ ΠΟΥ ΕΧΟΥΝ 0 ΥΠΟΛΟΙΠΟ Ή ΑΥΤΑ ΠΟΥ ΕΧΟΥΝ ΗΔΗ ΦΩΤΟΓΡΑΦΙΕΣ (ΓΙΑΤΙ ΕΙΝΑΙ ΗΔΗ ΑΝΑΡΤΗΜΕΝΑ ΣΤΟ SITE)
       R29.Open "SELECT DISTINCT KODE FROM EGGTIM G inner JOIN EID E ON G.KODE=E.KOD WHERE E.PROM LIKE '" + B + "%' and   LEFT(ATIM,1) IN ('G','g') AND (E.NUM3 is null or  E.NUM3='2')  AND E.POS01+E.POS02+E.POS03+E.POS04>0 AND (LEN(E.PIC0)<2  OR E.PIC0 IS NULL)   ORDER BY KODE", Gdb, adOpenDynamic, adLockOptimistic
       
       ' r2.Open "SELECT DISTINCT KODE FROM EGGTIM inner JOIN EID ON EGGTIM.KODE=EID.KOD WHERE   LEFT(ATIM,1) IN ('G','g') AND EID.NUM3<>2 AND  KODE NOT IN (SELECT KOD FROM EID WHERE POS01+POS03<=0 OR LEN(PIC0)>2) AND  HME>='" + Format(CDate(b), "MM/DD/YYYY") + "' ORDER BY KODE", Gdb, adOpenDynamic, adLockOptimistic
       
       
       Open "C:\MERCVB\NEW_PRODUCTS.TXT" For Output As #1
       Do While Not R29.EOF
              Print #1, R29!KODE
              R29.MoveNext
       Loop
       Close #1
       ...
       'EGGTIM2.RecordSource = "SELECT DISTINCT KODE FROM EGGTIM WHERE   LEFT(ATIM,1) IN ('G','g') AND  KODE NOT IN (SELECT KOD FROM EID WHERE POS01+POS03<=0 OR LEN(PIC0)>2) AND  HME>='" + Format(CDate(b), "MM/DD/YYYY") + "' ORDER BY KODE" '
       EGGTIM2.RecordSource = "SELECT DISTINCT KODE FROM EGGTIM G inner JOIN EID E ON G.KODE=E.KOD WHERE  E.PROM LIKE '" + B + "%' and    LEFT(ATIM,1) IN ('G','g') AND E.NUM3<>'2' AND E.POS01+E.POS02+E.POS03+E.POS04>0 AND (LEN(E.PIC0)<2  OR E.PIC0 IS NULL)  ORDER BY KODE"
       '"SELECT DISTINCT KODE FROM EGGTIM WHERE  LEFT(ATIM,1) IN ('G','g')  AND KODE NOT IN (SELECT KOD FROM EID WHERE LEN(PIC0)>2) AND  HME>='" + Format(CDate(b), "MM/DD/YYYY") + "' ORDER BY KODE"
       EGGTIM2.Refresh
...
'
    ...
    'UPDATE_CONTROLS  ΥΠΗΡΧΕ 13/4/2016
     ' ΜΕΤΑ ΣΕ ΑΝΤΙΚΑΤΑΣΤΑΣΗ ΜΠΗΚΑΝ ΟΙ ΠΑΡΑΚΑΤΩ 3 ΕΝΤΟΛΕΣ
     '   Set msEditor21.mDataSource = EID  ' for code you hav mum length.
      '  msEditor21.mDataField = "MEMO" '  "nNotes"     ' The field of your text
      '  msEditor21.Enabled = True
...
'     Dim mms As msEditor2
'     Set mms.mDataSource = EID
'     mms.mDataField = "MEMO"
'     mms.Enabled = True
...
130    ' m = Trim(mID(CommonDialog1.FILENAME, InStrRev(CommonDialog1.FILENAME, "\") + 1, 30))
       ' m = "a" + kodlabel.Caption + "-" + m  ' CommonDialog1.FILENAME ' Text1(0).Text
        ...
        m = toGreekLish(FONO.Text) + "-" + toGreekLish(EID.Recordset("ERG")) + cc0 + ".JPG"
...
Sub ftp_Eikones(strLocal As String, strRemote As String)
    ...
    If m2ftp.OpenConnection("toys-shop.gr", "toysftp", "Vykq819*") Then
