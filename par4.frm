...
240     sql = "SELECT LEFT(ATIM,1)+' '  +TITLOS AS [ΠΑΡΑΣΤΑΤΙΚΟ] ,"
       ' '''''''''''''''''''''''''''''''''''   & " CONVERT(DECIMAL(10,2),sum(CASE WHEN ascii(LEFT(ATIM,1)) IN (" + pol + ") THEN AJ2 ELSE -(AJ2)  END )) AS [" + Format(g_Fpa(2), "##") + "%], "
         ...
         '  sql = sql + " CONVERT(DECIMAL(10,2),sum(CASE WHEN ascii(LEFT(ATIM,1)) IN (" + pol + ") THEN AJ1+AJ2+AJ3+AJ4+AJ5+AJ6+AJ7 ELSE -(AJ1+AJ2+AJ3+AJ4+AJ5+AJ6+AJ7)  END )) AS [ΣΥΝΟΛΟ], "
           If poliseis = vbYes Then
              sql = sql + " (SELECT substring(min(ATIM),2,6) FROM DOKTIMATIM WHERE LEFT(ATIM,1)=PARASTAT.EIDOS) AS [ΑΠΟ ΠΑΡ.],"
             sql = sql + "(SELECT substring(MAX(ATIM),2,6) FROM DOKTIMATIM WHERE LEFT(ATIM,1)=PARASTAT.EIDOS ) AS [ΤΕΛ.ΠΑΡ],"
               sql = sql + "(SELECT STR(COUNT(ATIM)) FROM DOKTIMATIM WHERE LEFT(ATIM,1)=PARASTAT.EIDOS ) AS [ΠΛΗΘΟΣ],"
          ...
          sql = sql + " CONVERT(DECIMAL(10,2),sum(CASE WHEN ascii(LEFT(ATIM,1)) IN (" + pol + ") THEN KERDOS ELSE 0  END )) AS [ΚΕΡΔΟΣ] " _
           ...
           & "  GROUP BY LEFT(ATIM,1),TITLOS,PARASTAT.EIDOS"
...
For KOL = 1 To MSFlexGrid2.ColS
        ...
        'Else
            MSFlexGrid2.TextMatrix(k - 1, KOL) = Format(SUMA(KOL), "###,###,##0.00")
        'End If
        If SUMA(KOL) = 0 Then
            MSFlexGrid2.ColWidth(KOL) = 0
        Else
