...
Sub UPDATE_CONTROLS()
 ...
 For N = 0 To 6
    If Len(PIC(N).Caption) > 0 Then
       ...
       foto2(N).Picture = LoadPicture("\\Tsaftsufserver\images\" + PIC(N).Caption)
...
150        m = m + ".JPG"
        End If
160     FileCopy CommonDialog1.FILENAME, "\\Tsaftsufserver\images\" + m
    
170      foto2(Index).Picture = LoadPicture("\\Tsaftsufserver\images\" + m)
...
gdblag.Execute "update products set products_status=" + energo + ",manufacturers_id=" + manuf + ",products_price=" + mTimh + ",products_weight=" + str(Val(WEIGHT)) + "  where products_id=" + m_ID
...
For k = 0 To 6
   ...
   If Len(PIC(k).Caption) > 0 Then
      ' ftp_Eikones "c:\mercvb\images\" + PIC(k).Caption, "httpdocs/images/" + PIC(k).Caption
      Print #file, "send \\Tsaftsufserver\images\" + PIC(k).Caption
      nc = nc + 1
   End If
Next
