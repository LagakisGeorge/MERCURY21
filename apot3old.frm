...
Do While Not EGGTIM.EOF
  ...
  d = EGGTIM("HME")
  k = 0
  R.Open "SELECT * FROM EGGTIM WHERE KODE='" + A + "' AND LEFT(ATIM,1) IN ('G','g','λ') AND HME<='" + Format(d, "MM/DD/YYYY") + "' ORDER BY HME DESC", Gdb, adOpenDynamic, adLockOptimistic
  If R.EOF Then
...
Do While Not EGGTIM.EOF
   ...
   If Not IsNull(EGGTIM("kode")) Then
       Gdb.Execute "UPDATE EID SET XTI=" + Str(nNull(EGGTIM("TIMM") * (1 - EGGTIM("EKPT") / 100))) + ",CH1='" + atim + "',HPAR='" + Format(d, "MM/DD/YYYY") + "' where KOD='" + A + "'", n
   End If
