...
Begin VB.Form apot2vmlagakis 
   ...
   Begin RichTextLib.RichTextBox Rich1 
      ...
      _Version        =   393217
      Enabled         =   -1  'True
      TextRTF         =   $"apot2vmlagakis.frx":0000
   End
   ...
   End
End
Attribute VB_Name = "apot2vmlagakis"
...
Sub UPDATE_CONTROLS()
 ...
 For N = 0 To 6
    If Len(PIC(N).Caption) > 0 Then
       ...
       foto2(N).Picture = LoadPicture("c:\mercvb\images\" + PIC(N).Caption)
       StretchSourcePictureFromPicture foto2(N).Picture, foto2(N)
    Else
       foto2(N).Picture = LoadPicture()
...
120     CommonDialog1.ShowOpen
        Dim m As String
        On Error Resume Next
130     m = "a" + Trim(kodlabel.Caption) + "-" + Format(Index, "0") ' Text1(0).Text
140     If F_EIKONA = 11 Then
150        m = m + ".jpeg"
        End If
160     FileCopy CommonDialog1.FILENAME, "c:\mercvb\images\" + m
170     foto2(Index).Picture = LoadPicture("c:\mercvb\images\" + m)
180     StretchSourcePictureFromPicture foto2(Index).Picture, foto2(Index)
        PIC(Index).Caption = m
        '<EhFooter>
        Exit Sub
...
Command14_Click_Err:
        ...
        SAVE_ERROR Err.Description & " in ADOMERCNEW.Command14_Click " & "at line " & Erl
        Resume Next
        '</EhFooter>
...
gdblag.Close
gdblag.Open f_connect  ' "DRIVER={MySQL ODBC 5.1 Driver};SERVER=toys-shop.gr ;PORT =3306;DATABASE=toyshop;UID=toys;Password=doxadramas78"
...
Do While Not r.EOF
   ...
   If Not rl.EOF Then
        Gdb.Execute "INSERT INTO PRODUCT_ATTRIBUTES " _
      & "(KOD,OPTIONS_ID,OPTIONS_VALUES_ID,PRODUCT_ID) " _
      & " VALUES ('" + KOD + "',1," + str(r!options_values_id) + ",0);"
   End If
   rl.Close
N = N + 1
       DoEvents
...
' 104 gdblag.Open "DRIVER={MySQL ODBC 5.1 Driver};SERVER=db5.papaki.gr;PORT =3306;DATABASE=vasi;UID=lagakis;Password=3898339214231"
...
' virtue mart 1.1.19
'140  sql = "SELECT c.category_id as ID,x.category_parent_id as PARENT,c.category_name as NAME FROM jos_vm_category c inner join jos_vm_category_xref  x on c.category_id=x.category_child_id ORDER BY x.category_parent_id ,c.category_id "

'vm 2.0.24
140 sql = "SELECT a.virtuemart_category_id as ID,b.category_parent_id as PARENT,a.category_name as NAME FROM jsnrw_virtuemart_categories_el_gr a inner join jsnrw_virtuemart_category_categories b on a.virtuemart_category_id=b.category_child_id"
...
168       Gdb.Execute "INSERT INTO CATEGORIESANT (ID,PARENT,NAME) VALUES (" + str(r!ID) + "," + str(r!Parent) + ",'" + Replace(r!Name, "'", "''") + "')"
170       r.MoveNext
    Loop
172 r.Close
...
Form_Load_Err:
        MsgBox Err.Description & vbCrLf & _
               "in ADOMERCNEW.apot2vm.Form_Load " & _
               "at line " & Erl, _
               vbExclamation + vbOKOnly, "Application Error"
        Resume Next
        '</EhFooter>
...
m = "a" + Trim(kodlabel.Caption) + "-" + m  ' CommonDialog1.FILENAME ' Text1(0).Text
140     If F_EIKONA = 11 Then
150       ' m = m + ".JPG"
        End If
        m = Replace(m, ".jpg", ".jpeg")
160     FileCopy CommonDialog1.FILENAME, "c:\mercvb\images\" + m
    
170      foto2(Index).Picture = LoadPicture("c:\mercvb\images\" + m)
            
180     StretchSourcePictureFromPicture apot2vm.foto2(Index).Picture, apot2vm.foto2(Index)
        PIC(Index).Caption = m
...
FOTO2_DBLClick_Err:
        'MsgBox Err.Description & vbCrLf & _
               ...
               "at line " & Erl, _
               vbExclamation + vbOKOnly, "Application Error"
        SAVE_ERROR Err.Description & " in ADOMERCNEW.apot2vm.FOTO2_DBLClick " & "at line " & Erl
        Resume Next
        '</EhFooter>
...
NEA.Visible = True
 ...
 a = "insert into EID (KOD) VALUES ('" + KOD.Text + "')"
...
If m_new = 1 Then
   ' Exit Sub
    ...
    gdblag.Execute "INSERT INTO jsnrw_virtuemart_product_prices (virtuemart_product_id,virtuemart_shoppergroup_id,product_price,override,product_override_price,product_tax_id,product_discount_id,product_currency,product_price_publish_up,product_price_publish_down,price_quantity_start,price_quantity_end,created_on,created_by,modified_on,modified_by,locked_on,locked_by) VALUES " _
       & "(" + m_ID + ",0,'" + Format(Val(WEBPRICE.Text), "####.00000") + "',0,'0.00000'," + FPA + ",0,47,'2013-11-01 00:00:00','0000-00-00 00:00:00',0,0,'2013-11-03 16:35:41',42,'2013-11-03 16:44:17',42,'0000-00-00 00:00:00',0);"
Else
