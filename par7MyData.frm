...
Public Function OLDkillClient(ByVal park2garage3 As Integer, _
...
"""extra"": {""altCustAddress"": null,""altCustName"": null,""customerActivity"": ""ΧΟΝΔΡΙΚΟ ΕΜΠΟΡΙΟ ΗΛΕΚΤΡΟΝΙΚΩΝ ΥΠΟΛΟΓΙΣΤΩΝ, ΠΕΡΙΦΕΡΕΙΑΚΟΥ ΕΞΟΠΛΙΣΜΟΥ ΥΠΟΛΟΓΙΣΤΩΝ ΚΑΙ ΛΟΓΙΣΜΙΚΟΥ"",""customerCity"": ""ΘΕΣΣΑΛΟΝΙΚΗ"",""customerCode"": null,""customerDoyCode"": 4216,""customerDoyName"": ""ΣΤ ΘΕΣΣΑΛΟΝΙΚΗΣ"",""customerEmail"": ""info@bratnet.gr"",""customerName"": ""ΤΕΣΤ ΠΕΛΑΤΗΣ"",""customerPhone"": ""2310221222"",""customerSendEmail"": false,""customerStreetName"": ""ΛΑΜΠΡΑΚΗ 150"",""customerTk"": ""54352"",""customerVat"": ""026883248"",""destinationAddress"": null,""hotelEndDate"": null,""hotelRoom"": null,""hotelStartDate"": null,""invoiceRemarks"": null,""invoiceTypeName"": ""Τιμολόγιο Παροχής Υπηρεσιών"",""loadingAddress"": null,""movePurpose"": null,""nationalID"": null,""nspCode"": ""01"",""paymentMethodName"": ""Πίστωση 30 ημερών"",""salerActivity"": ""ΕΜΠΟΡΙΑ ΛΟΓΙΣΜΙΚΟΥ"",""salerAdditionalStreetName"": ""ΑΝΩ ΤΟΥΜΠΑ"",""salerCity"": ""ΘΕΣΣΑΛΟΝΙΚΗ"",""salerDoyCode"": 4216,""salerDoyName"":"
    ...
    ' JSON Body
    jsonBody = "{" & """externalSystemId"": ""4053""," & """issuerVatNumber"": ""028783755""," & """invoiceIssueDate"": ""2026-03-23""," & """invoiceIssueTime"": ""15:50:25""," & """companyBranch"": 0," & """invoiceType"": ""1.4""," & """invoiceSeries"": ""\u03A4\u0394\u0391\u03A4\u03A7""," & """netValue"": 20.0," & """vatAmount"": 0.0," & """totalValue"": 20.00," & """paymentAmount"": 20.00," & """terminalId"": ""54888913""," & """nspCode"": ""01""" & "}"
...
Public Function OLD_RELATECLIEND(ByVal MARK As String, _
                                 ByVal CORID As String, _
                                 mark1fim2 As Integer, _
                                 mfimaa As String, _
                                 mfimdate As String, _
                                 ByVal artam As String) As String
...
Function FERE_TRANSACTIONID(mID_NUM As String, _
                            ByRef answ As String) As String 'result@transcactionid
...
Private Sub ENARXI2APOSTOLIS(ByVal carrierAFM As String, _
                             ByVal qrl As String, _
                             ByVal ark As String)
...
Public Function READ_xml(ByVal v As String, _
                         mNode As String, _
                         mName As String, _
                         ByRef lathos As String) As String
...
Public Sub printUSNode(Node As IXMLDOMNode, ByRef AFM As String) '----------  ΠΡΟΣΟΧΗ --------------------
        ' οι παρακατω μεταβλητες αλλαζουν τιμες και χρησιμοποιουνται και εξω απο την SUB printUSNode
        'afm0,f_qrCodeUrl , MARK ,AFM,issueDate,AA,invoiceType,paytype,payaji,classificationType,totalNetValue,totalVatAmount
        '
...
170     sql = sql + "ISNULL(PEL.XRVMA,'') AS TK,SXETMARK,ISNULL(PARAKRATISI,0) AS PARAKRAT,ISNULL(B_C2,'') AS TYPOSPARAKRAT,isnull(EXCHANGERATE,1) AS EXCHANGERATE,isnull(CURRENCY,'EUR') AS CURRENCY ,ISNULL(ELGA,0) AS ELGA,isnull(KR1,0) AS KR1,ISNULL(KR2,0) AS KR2,PEL.EIDOS AS EIDOS,isnull(ANASTOLHFPA,0) as ANASTOLHFPA ,OTHERMOVEPURPOSETITLE, ORA,AYTOK,ISNULL(SXETMARKS,',') AS SXETMARKS,SKOPOS,AKYROMENO,isnull(JWT,'') AS JWT, "
        sql = sql + "STREET,TIM.STREETNUMBER,POSTALCODE,CITY,ISNULL(TIM.BRANCH,'0') AS BRANCH,ISNULL(HMEPARAD,HME) AS HMEPARAD,ISNULL(ORAPARAD,'00:00') AS ORAPARAD,FORTOSH,ISNULL(PARAT,'') AS PARAT,ISNULL(ISANAPODO,1) AS ISANAPODO,ISNULL(TOWEIGH,0) AS TOWEIGH,ISNULL(ANAPODOAITIA,0) AS ANAPODOAITIA,ISNULL(AFMTRITOU,'') AS AFMTRITOU "
...
480                             Set elem2Field = docStock.createElement("movePurpose"): elem2Field.Text = Left(SQLDT("SKOPOS"), 2): elemField.appendChild elem2Field
                            ...
                            If Len(SQLDT("AFMTRITOU")) > 7 Then ' isDiakin = 2 And (Val(Left(SQLDT("SKOPOS"), 2)) = 2 Or (Split(ctypos, ";")(0) = "10.2" Or Split(ctypos, ";")(0) = "10.1")) Then
                                ...
                                'old M_AFMGRAND = GGET_CVALUE("SELECT ISNULL(AFMGRANDPEL,'0') FROM PARASTAT WHERE EIDOS='" + Left(SQLDT("ATIM"), 1) + "'")
                                ...
                                '8.20 Κατηγορία Οντότητας (EntityType) Κωδικός Περιγραφή Παρατηρήσεις 1
...
512                             Set elem2Field = docStock.createElement("isDeliveryNote"): elem2Field.Text = "true": elemField.appendChild elem2Field
                           ...
                           ' 8.21 Αιτία Έκδοσης Αντίστροφης Διακίνησης Κωδικός Περιγραφή
                                   '1 ΜΗ ΥΠΟΧΡΕΟΣ ΕΚΔΟΣΗΣ
                                   '2 ΑΡΝΗΣΗ ΕΚΔΟΣΗΣ/ΕΚ ΠΑΡΑΔΡΟΜΗΣ ΜΗ ΕΚΔΟΣΗ
                                   '3 ΕΝΔΟΚΟΙΝΟΤΙΚΗ ΑΠΟΚΤΗΣΗ
                                   '4 ΑΠΟΚΤΗΣΗ ΤΡΙΤΗ ΧΩΡΑ 5 ΑΝΤΙΣΤΡΟΦΗ ΥΠΟΧΡΕΩΣΗΣ
...
520                                 Set elem2Field = docStock.createElement("selfPricing"): elem2Field.Text = "true": elemField.appendChild elem2Field
                            ...
                            Else
                                ...
                                If Left(mTypPar, 3) = "3.1" Then ' ΤΙΤΛΟΣ ΚΤΗΣΗΣ (ΑΠΟ ΠΩΛΗΣΕΙΣ)
                                Else
...
Sub MAKE_request(TXML As String, _
                 ...
                 noask As Integer, _
                 ByVal file_id_num As String, _
                 ByRef markReal As String) '(TXML As MSXML2.DOMDocument)
...
150     For Each Node In nodeList  ' ΓΙΑ ΚΑΘΕ INVOICE -------------------------------------------------------

152         Print Node.nodeName  ' this works'
154         List1.AddItem Node.nodeName + "*******" + Node.Text  ' INVOICE
       
            '       Print node.n
            'PARE TO INVOICE KAI BGALE TA KOMMATIA POY ME ENDIAFEROYN
...
498                         SJ = SJ + Chr(13) + ",""currency"":""" + "EUR" + """"
                        ...
                        SJ = SJ + Chr(13) + ",""invoiceNote"":""" + SQLDT("PARAT") + """"
...
Sub SendToParochos(SJ As String, _
                   id_num, _
                   posSignature As String, _
                   TID As String, _
                   POS_AMOUNT As Single, _
                   POSTransactionId As String, _
                   IS_POS As Integer, _
                   ubl As String, _
                   ByVal UPLOAD As Integer)
...
Public Function POS_submitpayment(ByVal MARK As String, _
                                  ByVal extid As String, _
                                  ByVal signatureParochos As String, _
