VERSION 5.00
Begin VB.Form PAR112 
   BackColor       =   &H00C0C000&
   Caption         =   "Êïóôïëüãçóç Áãïñþí"
   ClientHeight    =   3150
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7785
   ...
   Begin VB.Label FPAEID 
      ...
      Width           =   45
   End
   Begin VB.Label ltiproekptosis 
      BackStyle       =   0  'Transparent
      Caption         =   "ëô ðñï åêðôùóçò"
      Height          =   255
      Left            =   240
      TabIndex        =   24
      Top             =   2760
...
Dim F_TIMH_PRO_EKPTOSHS As Integer
...
100     MPER = PAR2.EID.Recordset("ONO")

110     mtimes = InputBox("Ðüóåò åôéêÝôåò;" + PAR2.EID.Recordset("ONO"), , POSO.Caption)
120     TIMM = Format(Val(Text1(4).Text), "##0.00")
130     timologio = "" ' PAR2.EID.Recordset("KOD") + Left(PAR2.Text1.Text, 6) + "=" + PAR2.PEL.Recordset("KOD")
140     timologio = timologio + Format(Val(label11.Text) * (100 + g_Fpa(2)) / 100, "####.00") + "-" + Format(PAR2.DTPicker1, "dd/mm/yyyy")

        Dim mlabel

150     If IsNull(PAR2.EID.Recordset("ERG")) Then
160         GoSub FIND_N_NUMBER
        Else

170         If Val(PAR2.EID.Recordset("ERG")) = 0 Then
180             GoSub FIND_N_NUMBER
            Else
190             mlabel = PAR2.EID.Recordset("erg")
            End If
...
450     Gdb.Execute "UPDATE EID SET ERG='" + mlabel + "' WHERE KOD='" + PAR2.EID.Recordset("KOD") + "'"
460     Return
...
110     mtimes = 1
120     TIMM = Format(Val(Text1(4).Text), "##0.00")
130     MPER = PAR2.EID.Recordset("ONO")
...
250     Printer.Print "  " + PAR2.EID.Recordset("ERG") + "-" + Format(500 + Val(Text1(2).Text), "####0") + " -" + PAR2.EID.Recordset("KOD")
...
360     Gdb.Execute "UPDATE EID SET ERG='" + mlabel + "' WHERE KOD='" + PAR2.EID.Recordset("KOD") + "'"
370     Return
...
160     If IsNull(PAR2.EID.Recordset("ERG")) Then
170         mERG = "--"
        Else
180         mERG = PAR2.EID.Recordset("ERG")
        End If

190     If IsNull(PAR2.EID.Recordset("KOD")) Then
200         Me.Caption = "NULL KOD"
        End If

210     If IsNull(Replace(PAR2.EID.Recordset("ONO"), " ", "_")) Then
...
230     If IsNull(PAR2.EID.Recordset("LTI5")) Then
...
250     If IsNull(PAR2.EID.Recordset("POS_KERD")) Then
...
270     If IsNull(PAR2.EID.Recordset("ERG")) Then
...
290         If Len(PAR2.EID.Recordset("ERG")) = 0 Then
300             mERG = "--"
            End If
        End If
...
320     T = T + PAR2.EID.Recordset("KOD") + " "
330     T = T + Replace(PAR2.EID.Recordset("ONO"), " ", "_") + " "
...
420         PRINT_ETIK
        ...
        '   DUM = Shell("c:\lageuro\makisget.exe " + par2.EID.Recordset("KOD") + " " + Replace(par2.EID.Recordset("ONO"), " ", "_") + " " + Format(par2.EID.Recordset("LTI5"), "##0.00") + " " + Format(par2.EID.Recordset("POS_KERD"), "##0") + " " + mERG)
...
Private Sub Command2_Click()
        ...
        stopKost.Caption = "1"
100     'PAR2.kostologhsh.Value = False
            PAR2.exodos.Caption = "1"
110     Unload Me
...
100     F_XONDRIKH = Val(FINDPARAMETROI(1, "PAR11", "F_XONDRIKH", "1", "÷ïíäñéêÞ ôéìÞ=1-3/Ï÷é=0"))   '
110     F_ETTIKETES = Val(FINDPARAMETROI(1, "PAR11", "F_ETTIKETES", "0", "AYTOMATA ÓE EKTYÐÙÓÇ=1/Ï÷é=0"))   '

F_TIMH_PRO_EKPTOSHS = Val(FINDPARAMETROI(1, "PAR11", "F_TIMH_PRO_EKPTOSHS", "0", "ÐÑÏÔÅÉÍÏÌÅÍÇ ËÉÁÍÉÊÇ ËÔ ÐÑÏ ÅÊÐÔÙÓÇÓ+ÖÐÁ=1/Ï÷é=0"))   '
...
Private Sub Text1_GotFocus(Index As Integer)
        ...
        ' p.x. 1.19  gia 19%
100     SyntFpa = (1 + g_Fpa(gVal(FPAEID.Caption)) / 100)
...
If F_TIMH_PRO_EKPTOSHS = 1 Then

    'óáí ðñïôåéíüìåíç ëéáíéêç íá ÷ñçóéìïðïéåß ôçí ôéìç ðñï åêðôùóçò +  öðá
    protLmeFpa.Caption = Format(gVal(ltiproekptosis) * SyntFpa, "#####.00")
Else
         'proteinomeni lianikh me fpa
260      protLmeFpa.Caption = Format((100 + exoda) / 100 * (100 + Val(Text1(2).Text)) / 100 * TimXon * SyntFpa, "#####.00")
...
100     mONOMA = PAR2.EID.Recordset("ONO")    ' Text1(1).Text 'ONOMA
110     mBarcode = PAR2.EID.Recordset("ERG")    'Text1(0).Text '  ' BARCODE
120     mERG = PAR2.EID.Recordset("KOD")    ' Text1(6).Text ' erg
