...
Q = "select  KODE,D.ONO ,"
Q = Q + "sum(IIF(LEFT(G.ATIM,1)='λ' ,XRE-PIS,0) )                      AS [ΠΟΣ.ΑΠΟΓ] ,"
Q = Q + "SUM(IIF(LEFT(G.ATIM,1)='λ' ,(XRE-PIS)*TIMM,0) )          AS [ΑΞΙΑ ΑΠΟΓ] ,"

'ΑΓΟΡΕΣ ΠΟΣΟΤΗΤΑ & ΑΞΙΑ
Q = Q + "sum(IIF(LEFT(G.ATIM,1) in (" + PosAg + ") , IIF(LEFT(G.ATIM,1) IN (" + PosAgEp + ")  ,-POSO,POSO),0) )            AS [ΠΟΣ.ΑΓΟΡ] ,"
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ," + AjAg + ")>0  ,IIF(LEFT(G.ATIM,1)=" + AjAgEp + ",-1,1)*(POSO)*TIMM*(100-G.EKPT)*(100-G.EKPT2)/10000,0 ) ) AS [ΑΞΙΑ AΓΟΡ],"

'ΠΩΛΗΣΕΙΣ ΠΟΣΟΤΗΤΑ & ΑΞΙΑ
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ," + PosPol + ")>0  ,IIF(LEFT(G.ATIM,1) IN (" + PosPolEp + ")  ,-POSO,POSO) ,0 ) ) AS [ΠΩΛΗΘ.ΠΟΣ],"
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ," + AjPol + ")>0  ,IIF(LEFT(G.ATIM,1) IN (" + AjPolEp + ")  ,-POSO,POSO)*TIMM*(100-G.EKPT)*(100-G.EKPT2)/10000,0 ) ) AS [ΑΞ.ΠΩΛ],"
'ΥΠΟΛΟΙΠΟ
Q = Q + "sum(IIF(LEFT(G.ATIM,1)='λ' ,XRE-PIS,0) ) + sum(IIF(LEFT(G.ATIM,1) in (" + PosAg + ") , XRE-PIS,0) ) -SUM(IIF (charindex(  LEFT(G.ATIM,1) ," + PosPol + ")>0  ,IIF(LEFT(G.ATIM,1) IN (" + PosPolEp + ")  ,-POSO,POSO) ,0 ) )  AS [ΥΠΟΛ.ΠΟΣ],"
'
Q = Q + "(sum(IIF(LEFT(G.ATIM,1)='λ' ,POSO*D.XTI,0) ) + sum(IIF(LEFT(G.ATIM,1) in (" + PosAg + ") , (XRE-PIS)*D.XTI,0) ) -SUM(IIF (charindex(  LEFT(G.ATIM,1) ," + PosPol + ")>0  ,IIF(LEFT(G.ATIM,1) IN (" + PosPolEp + ")  ,-POSO*D.XTI,POSO*D.XTI) ,0 ) )  ) AS [ΑΞΙΑ ΥΠΟΛ]"
