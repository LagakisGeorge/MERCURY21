...
'ΑΓΟΡΕΣ ΠΟΣΟΤΗΤΑ & ΑΞΙΑ PROHG
Q = Q + "sum(IIF(LEFT(G.ATIM,1) in (" + PosAg + "," + PosAgEp + ")  AND G.HME<'" + Format(apo.Value, "MM/DD/YYYY") + "' , IIF(LEFT(G.ATIM,1) IN (" + PosAgEp + ")  ,-POSO,POSO),0) )            AS [ΠΡΟΗΓ.ΠΟΣ.ΑΓΟΡ] ,"
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ,'" + Replace(AjAg + AjAgEp, "'", "") + "' )>0  AND G.HME<'" + Format(apo.Value, "MM/DD/YYYY") + "' ,IIF(LEFT(G.ATIM,1) in (" + AjAgEp + ")" + ",-1,1)*(POSO)*TIMM*(100-G.EKPT)*(100-G.EKPT2)/10000,0 ) ) AS [ΠΡΟΗΓ.ΑΞΙΑ AΓΟΡ], "



''ΠΩΛΗΣΕΙΣ ΠΟΣΟΤΗΤΑ & ΑΞΙΑ PROHG
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ,'" + Replace(PosPol + PosPolEp, "'", "") + "')>0   AND G.HME<'" + Format(apo.Value, "MM/DD/YYYY") + "' ,IIF(LEFT(G.ATIM,1) IN (" + PosPolEp + ")  ,-POSO,POSO) ,0 ) ) AS [ΠΡΟΗΓ.ΠΩΛΗΘ.ΠΟΣ],"
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ,'" + Replace(AjPol + AjPolEp, "'", "") + "')>0  AND G.HME<'" + Format(apo.Value, "MM/DD/YYYY") + "' ,IIF(LEFT(G.ATIM,1) IN (" + AjPolEp + ")  ,-POSO,POSO)*TIMM*(100-G.EKPT)*(100-G.EKPT2)/10000,0 ) ) AS [ΠΡΟΗΓ.ΑΞ.ΠΩΛ],"



''ΑΓΟΡΕΣ ΠΟΣΟΤΗΤΑ & ΑΞΙΑ PERIODOY
Q = Q + "sum(IIF(LEFT(G.ATIM,1) in (" + PosAg + "," + PosAgEp + ")  AND G.HME>='" + Format(apo.Value, "MM/DD/YYYY") + "' , IIF(LEFT(G.ATIM,1) IN (" + PosAgEp + ")  ,-POSO,POSO),0) )            AS [ΠΟΣ.ΑΓΟΡ ΠΕΡΙΟΔ] ,"
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ,'" + Replace(AjAg + AjAgEp, "'", "") + "' )>0  AND G.HME>='" + Format(apo.Value, "MM/DD/YYYY") + "' ,IIF(LEFT(G.ATIM,1) in (" + AjAgEp + ")" + ",-1,1)*(POSO)*TIMM*(100-G.EKPT)*(100-G.EKPT2)/10000,0 ) ) AS [ΑΞΙΑ AΓΟΡ ΠΕΡΙΟΔ], "
'
'''ΠΩΛΗΣΕΙΣ ΠΟΣΟΤΗΤΑ & AXIA PEROIODOY
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ,'" + Replace(PosPol + PosPolEp, "'", "") + "')>0  AND G.HME>='" + Format(apo.Value, "MM/DD/YYYY") + "' ,IIF(LEFT(G.ATIM,1) IN (" + PosPolEp + ")  ,-POSO,POSO) ,0 ) ) AS [ΠΩΛΗΘ.ΠΟΣ],"


Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ,'" + Replace(AjPol + AjPolEp, "'", "") + "')>0  AND G.HME>='" + Format(apo.Value, "MM/DD/YYYY") + "' ,IIF(LEFT(G.ATIM,1) IN (" + AjPolEp + ")  ,-POSO,POSO)*TIMM*(100-G.EKPT)*(100-G.EKPT2)/10000,0 ) ) AS [ΑΞ.ΠΩΛ],"
'
...
Q = Q + "WHERE T.AKYROMENO=0 AND charindex(  LEFT(G.ATIM,1) ,'λ," + Replace(PosAg + PosAgEp + PosPol + PosPolEp + AjAg + AjAgEp + AjPol + AjPolEp, "'", "") + "')>0"
      ...
      ALLTIME = " and G.HME>='" + Format(gEnarjh, "MM/DD/YYYY") + "' AND G.HME<='" + Format(gLhjh, "MM/DD/YYYY") + "'"
      Period = " and G.HME>='" + Format(apo.Value, "MM/DD/YYYY") + "' AND G.HME<='" + Format(eos.Value, "MM/DD/YYYY") + "'"
Q = Q + " AND G.HME<='" + Format(eos.Value, "MM/DD/YYYY") + "'"     '   Period  '                 "  AND G.HME>=@x1 and G.HME<@x2"
