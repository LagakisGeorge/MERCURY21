...
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ," + Replace(AjAg, "','", "") + ")>0  ,IIF(LEFT(G.ATIM,1) in (" + AjAgEp + ")" + ",-1,1)*(POSO)*TIMM*(100-G.EKPT)*(100-G.EKPT2)/10000,0 ) ) AS [ΑΞΙΑ AΓΟΡ], "

''ΠΩΛΗΣΕΙΣ ΠΟΣΟΤΗΤΑ & ΑΞΙΑ
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ,'" + Replace(PosPol, "'", "") + "')>0  ,IIF(LEFT(G.ATIM,1) IN (" + PosPolEp + ")  ,-POSO,POSO) ,0 ) ) AS [ΠΩΛΗΘ.ΠΟΣ],"
Q = Q + "SUM(IIF (charindex(  LEFT(G.ATIM,1) ,'" + Replace(AjPol, "'", "") + "')>0  ,IIF(LEFT(G.ATIM,1) IN (" + AjPolEp + ")  ,-POSO,POSO)*TIMM*(100-G.EKPT)*(100-G.EKPT2)/10000,0 ) ) AS [ΑΞ.ΠΩΛ],"
'ΥΠΟΛΟΙΠΟ
