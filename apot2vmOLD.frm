...
Private Sub Command4_Click()
    ...
    If a = "*" Then
       ...
       'ΝΑ ΜΗΝ ΕΜΦΑΝΙΖΕΙ ΑΥΤΑ ΠΟΥ ΕΧΟΥΝ 0 ΥΠΟΛΟΙΠΟ Ή ΑΥΤΑ ΠΟΥ ΕΧΟΥΝ ΗΔΗ ΦΩΤΟΓΡΑΦΙΕΣ (ΓΙΑΤΙ ΕΙΝΑΙ ΗΔΗ ΑΝΑΡΤΗΜΕΝΑ ΣΤΟ SITE)
       R28.Open "SELECT DISTINCT KODE FROM EGGTIM G inner JOIN EID E ON G.KODE=E.KOD WHERE KODE LIKE '" + b + "%' and   LEFT(ATIM,1) IN ('G','g') AND (E.NUM3 is null or  E.NUM3='2')  AND E.POS01+E.POS03>0 AND (LEN(E.PIC0)<2  OR E.PIC0 IS NULL)   ORDER BY KODE", Gdb, adOpenDynamic, adLockOptimistic
       ...
       'EGGTIM2.RecordSource = "SELECT DISTINCT KODE FROM EGGTIM WHERE   LEFT(ATIM,1) IN ('G','g') AND  KODE NOT IN (SELECT KOD FROM EID WHERE POS01+POS03<=0 OR LEN(PIC0)>2) AND  HME>='" + Format(CDate(b), "MM/DD/YYYY") + "' ORDER BY KODE" '
       EGGTIM2.RecordSource = "SELECT DISTINCT KODE FROM EGGTIM G inner JOIN EID E ON G.KODE=E.KOD WHERE  KODE LIKE '" + b + "%' and    LEFT(ATIM,1) IN ('G','g') AND E.NUM3<>'2' AND E.POS01+E.POS03>0 AND (LEN(E.PIC0)<2  OR E.PIC0 IS NULL)  ORDER BY KODE"
       '"SELECT DISTINCT KODE FROM EGGTIM WHERE  LEFT(ATIM,1) IN ('G','g')  AND KODE NOT IN (SELECT KOD FROM EID WHERE LEN(PIC0)>2) AND  HME>='" + Format(CDate(b), "MM/DD/YYYY") + "' ORDER BY KODE"
       EGGTIM2.Refresh
...
Dim R9 As New ADODB.Recordset
    ...
    R.Open "select  * from EID WHERE len(KOD)>1 AND LEN(PIC0)>2", Gdb, adOpenDynamic, adLockOptimistic
