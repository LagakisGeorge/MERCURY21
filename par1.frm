...
118         LASTTIMOL.RecordSource = "SELECT top 5000 HME,ATIM,AJI AS [AΞΙΑ],PEL.EPO AS [ΕΠΩΝΥΜΙΑ],PEL.KOD AS [ΚΩΔΙΚΟΣ],PARAT AS [ΣΧΟΛΙΑ],SXETIKO AS [ΣΧΕΤ.ΠΑΡ],ID_NUM,ENTITYMARK,SXETMARK FROM TIM INNER JOIN PEL ON TIM.EIDOS=PEL.EIDOS AND TIM.KPE=PEL.KOD where " + querpol + " and ATIM LIKE '%" + ar_paraggt.Text + "%' ORDER BY ID_NUM DESC"
        Else
120         If Len(querpol) > 2 Then querpol = " where " + Left$(querpol, 85)
122         LASTTIMOL.RecordSource = "SELECT top 5000 HME,ATIM,AJI AS [AΞΙΑ],PEL.EPO AS [ΕΠΩΝΥΜΙΑ],PEL.KOD AS [ΚΩΔΙΚΟΣ],PARAT AS [ΣΧΟΛΙΑ],SXETIKO AS [ΣΧΕΤ.ΠΑΡ],ID_NUM,ENTITYMARK,SXETMARK  FROM TIM INNER JOIN PEL ON TIM.EIDOS=PEL.EIDOS AND TIM.KPE=PEL.KOD " + querpol + " ORDER BY ID_NUM DESC"
        End If
