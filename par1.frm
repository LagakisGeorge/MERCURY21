...
1130        If InStr(f_AKYR_benzPAR, Left$(f_matim, 1)) > 0 Then
              ...
              If F_DIOR = 0 Then  ' στην διορθωση τιμολογιου να μην  βγαζει ξανα συνοδευτικο
                 ...
                 If f_hotel = 1 Then
                     forosDiamonis id_num, f_matim, "1111", "e", "ΠΕΛΆΤΗΣ ΛΙΑΝΙΚΗΣ"
...
118     PARAS.BackColor = vbWhite
        ...
        '   Set db = OpenDatabase(gDir, False, False, gConnect)
        'End If
120     R.Open "SELECT ISNULL(AFMGRANDPEL,'0') AS AFMGRANDPEL2,ISNULL(PROEPILFPA,0) AS PROEPILFPA2,ISNULL(DESMIA,'0') AS DESMIA2,*FROM PARASTAT", Gdb, adOpenDynamic, adLockOptimistic
...
430             Frame2(0).Height = Abs(Frame2(0).Top - Grid1.Top - 100)
...
128         Frame2(0).Height = Frame2(0).Height + 100  '+300
...
F_YCOS_FORM_METAF = FINDPARAMETROI(1, "PAR1", "F_YCOS_FORM_METAF", "1600", "Υψος φόρμας προεπ.1600 ")
...
f_SHOWALLAPOT = Val(FINDPARAMETROI(6, "PAR1", "f_SHOWALLAPOT", "1", "Προεπιλογή 1=Δειχνει ΠΑΡ/ΚΑ ΟΛΩΝ ΤΩΝ ΑΠΟΘΗΚΩΝ 0=ΜΟΝΟ ΤΡΕΧΟΥΣΑ ΑΠΟΘΗΚΗ"))
...
Sub forosDiamonis(ByVal id_num As Long, ATIM As String, mPELKOD As String, EIDOSNEO As String, mEpo As String) '1111
        ...
        'f_EID_AKYR_APOD  ΒΡΙΣΚΩ ΜΕ ΠΟΙΟ ΑΚΥΡΩΤΙΚΟ ΑΚΥΡΩΝΕΤΑΙ Π.Χ. "κ"
        ' f_TimhForoyDiuamonis
        ' EIDOSNEO AN TO PARAGOMENO ΘΑ ΕΙΝΑΙ ΣΤΙΣ ΠΩΛΗΣΕΙΣ 'e' ή ΣΤΙΣ ΑΓΟΡΕΣ 'r'
...
102     '  akyr = "p"
        ...
        '--------------------  ΑΝΟΙΓΩ ΤΟΝ ΠΕΛΑΤΗ ΛΙΑΝΙΚΗΣ ΑΝ ΔΕΝ ΥΠΑΡΧΕΙ ---------------------------
        Dim nc As Integer
104     nc = GET_NVALUE("select count(*) from PEL WHERE KOD='" + mPELKOD + "' AND EIDOS='" + EIDOSNEO + "'")
106     If nc = 0 Then
108         Gdb.Execute "insert into PEL (KOD,EIDOS,EPO,AFM) VALUES ('" + mPELKOD + "','" + EIDOSNEO + "','" + mEpo + "','000000000') "
        End If
...
116     atimakyr = akyr + Format(aritm2, Left$("0000000000", f_psifia_atim)) '  "κ900001" ' π.χ.
        ...
        '---------- ΒΓΑΖΩ ΑΝΤΙΓΡΑΦΟ ΤΟΥ ΚΥΡΙΟΥ ΠΑΡΑΣΤΑΤΙΚΟΥ DOKTIM , DOKEGGTIM , DOKEGG  ----------------------------------------------------------------
118     Gdb.Execute "select * into DOKTIMAKYR" + Format(id_num, "00000000") + " FROM TIM WHERE ID_NUM=" + str(id_num)
120     Gdb.Execute "select * into DOKEGGTIMAKYR" + Format(id_num, "00000000") + " FROM EGGTIM WHERE ID_NUM=" + str(id_num)
122     Gdb.Execute "select * into DOKEGGAKYR" + Format(id_num, "00000000") + " FROM EGG WHERE IDTIM=" + str(id_num)
...
'------- ΕΔΩ ΓΙΝΕΤΑΙ ΤΟ UPDATE ΤΩΝ DOK???. ΠΡΙΝ ΤΑ ΚΑΝΩ INSERT STHN LIVE ΒΑΣΗ -------------------------------------
132     Gdb.Execute "UPDATE  DOKTIMAKYR" + Format(id_num, "00000000") + " SET ENTITYMARK='',SXETIKO='" + ar_apod.Text + "', HME='" + cHME + "', PARAT='ΣΧ.ΑΠΟΔ." + ar_apod.Text + "', KLEIDI='" + atimakyr + "',ATIM='" + atimakyr + "'"
        ' ΔΕΝ ΤΑ ΚΑΝΩ ΑΡΝΗΤΙΚΑ ΓΙΑ ΝΑ ΠΑΝΕ MYDATA      ,AJ1=-AJ1,AJ2=-AJ2,AJ3=-AJ3,AJ4=-AJ4,AJ5=-AJ5,AJ6=-AJ6,AJ7=-AJ7,AJ8=-AJ8,AJ9=-AJ9,FPA1=-FPA1,FPA2=-FPA2,FPA3=-FPA3,FPA4=-FPA4,FPA6=-FPA6,FPA7=-FPA7,FPA8=-FPA8,FPA9=-FPA9,AJI=-AJI"
...
144     Gdb.Execute "UPDATE  DOKEGGTIMAKYR" + Format(id_num, "00000000") + " SET EIDOS='" + EIDOSNEO + "',KAU_AJIA=0,MIK_AJIA=0,TIMM=0,PELKOD='" + mPELKOD + "',HME='" + cHME + "',ATIM='" + atimakyr + "', ID_NUM=" + C_ID_NUM
...
'------- TELOS ΤΟ UPDATE ΤΩΝ DOK???. ΠΡΙΝ ΤΑ ΚΑΝΩ INSERT STHN LIVE ΒΑΣΗ -------------------------------------



'-------  INSERT STHN LIVE ΒΑΣΗ -------------------------------------
148     Gdb.Execute "INSERT INTO EGGTIM select * FROM DOKEGGTIMAKYR" + Format(id_num, "00000000"), L
