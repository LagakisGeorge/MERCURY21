...
Begin VB.Form pelat3param 
   ...
   Begin VB.ListBox List1 
      Height          =   2400
      ItemData        =   "pelat3param.frx":0014
      Left            =   240
      List            =   "pelat3param.frx":0016
   ...
   Begin Crystal.CrystalReport CrystalReport1 
      ...
      SelectionFormula=   $"pelat3param.frx":0018
   End
   Begin MSHierarchicalFlexGridLib.MSHFlexGrid GR1 
      Bindings        =   "pelat3param.frx":009D
      Height          =   1215
      Left            =   120
      TabIndex        =   6
      Top             =   9480
   ...
   Begin VB.ComboBox Combo2 
      Height          =   315
      ItemData        =   "pelat3param.frx":00C3
      Left            =   360
      List            =   "pelat3param.frx":00D3
   ...
   Begin VB.ComboBox COMBO_PELPROM 
      Height          =   315
      ItemData        =   "pelat3param.frx":0118
      Left            =   120
      List            =   "pelat3param.frx":0122
      Style           =   2  'Dropdown List
      TabIndex        =   0
      Top             =   3600
      Width           =   4095
...
Public Sub YPOLOGISMOSparam()

   Data1.ConnectionString = gConnect
   
   
   
580     Data1.RecordSource = Rep_Fields.Text + WHERE_QUERY.Text
...
350         x = x + " from PEL where  KOD NOT LIKE '997%'  AND KOD NOT LIKE '998%'  AND EIDOS='" + IIf(COMBO_PELPROM.Text = COMBO_PELPROM.List(0), "e", "r") + "'"
        Else
360         x = x + " from PEL where PEL.KOD NOT IN (SELECT LTRIM(STR(TIMH)) FROM PINAKES WHERE (NOT TIMH IS NULL) AND  TYPOS=12) AND EIDOS='" + IIf(COMBO_PELPROM.Text = COMBO_PELPROM.List(0), "e", "r") + "'"
        End If
...
570         Text2.Text = x
 ...
 x = Rep_Fields.Text + WHERE_QUERY.Text '. 'ξεκινάω με το αρχικό
...
If True Then 'synola.Value = vbChecked Then
   Dim SUMES2(40) As Double

         Do While Not Data1.Recordset.EOF

             For k = 0 To Data1.Recordset.FIELDS.Count - 1

                 If mID(sumes.Text, k + 1, 1) = "1" Then  ' IsNumeric(DATA1.Recordset.FIELDS(k).Value) And DATA1.Recordset.FIELDS(k).Type <> 202 Then
         ...
         For k = 0 To Data1.Recordset.FIELDS.Count - 1
             ...
             If SUMES2(k) > 0 Then
                 TDBGrid.Splits(0).columns(k).FooterText = Format(SUMES2(k), "######0.00")
             End If
...
770         PELA = PELA + "     AΠΟ " + Format(apo, "DD/MM/YYYY") + " ΕΩΣ " + Format(eos, "DD/MM/YYYY")
            ...
            On Error GoTo YPOLOGISMOS_Click_Err    ' Resume Next
...
790         print3_xar x, mSYN, PELA, 0    'typos2
        ...
        For k = 0 To Data1.Recordset.FIELDS.Count - 1
           ' TDBGrid.Splits(0).columns(k).Width = Min(7, DATA1.Recordset.FIELDS(k).ActualSize) * 200
        Next
