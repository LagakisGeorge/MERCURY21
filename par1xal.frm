...
Dim F_SYMPYKNVSH 'ΣΥΜΠΥΚΝΩΣΗ ΙΔΙΩΝ ΚΩΔΙΚΩΝ ΣΤΟ ΤΙΜΟΛΟΓΙΟ
...
Dim f_mono_apoParaggelia As Integer ' τιμολογηση μόνο από παραγγελίες
...
20      re.AddNew
        ...
        re("timm") = Val(Grid1.TextMatrix(K, F_T))
         ...
         Else
            ...
            If Val(Grid1.TextMatrix(K, f_p)) <> XROMATA(K, K2, k3) Then
               re("poso") = Val(Grid1.TextMatrix(K, f_p))
            Else
...
R.Open "SELECT *FROM EGG", Gdb, adOpenDynamic, adLockOptimistic
...
R.Open "select POL,TITLOS,EIDOS from PARASTAT where POL='" + X + "' order by TITLOS", Gdb, adOpenDynamic, adLockOptimistic
...
If f_TIMOKAT > 0 And Not IsNull(PEL.Recordset("pek")) Then
   R.Open "select EKPT FROM TIMOKAT WHERE TIMOK=" + Str(PEL.Recordset("pek")) + " AND KOD='" + Left(EID.Recordset("kod"), 13) + "'", Gdb, adOpenForwardOnly, adLockReadOnly
   If R.EOF Then
      Grid1.Text = ""
   Else
      Grid1.Text = R(0)
...
If Len(Dir(PI)) > 0 Then
  ...
  ' Picture2.Picture = LoadPicture(PI)
   On Error Resume Next
...
140
   ...
   Loop
   Grid1.row = 1
   If K > 1 Then TIMText3.Text = Grid1.TextMatrix(1, f_k)
   F_DIOR = 1
...
' par1.SHOW
 ...
 F_SYMPYKNVSH = Val(FindParametroi("PAR1", "F_SYMPYKNVSH", "0", "Γ. 5. ΣΥΜΠΥΚΝΩΣΗ ΙΔΙΩΝ ΚΩΔΙΚΩΝ=1")) 'posa psifia tha exei h kathe seira
...
f_Desmia = Val(FindParametroi("PAR1", "F_DESMIA", "0", "Γ. 3.ΟΙ ΠΑΡΑΓΓΕΛΙΕΣ ΥΠΟΛΟΓΙΖΟΝΤΑΙ ΣΤΟ ΥΠΟΛΟΙΠΟ=1 ")) ' χρησιμοποιω τις παραγγελίες για έλεγχο υπολοίπου  'βλεπε mdesmia
                        ...
                        'Dim ans As Integer
 'Dim f_mono_apoParaggelia As Integer ' τιμολογηση μόνο από παραγγελίες
f_mono_apoParaggelia = Val(FindParametroi("PAR1", "f_mono_apoParaggelia", "0", "τιμολογηση μόνο από παραγγελίες")) ' χρησιμοποιω τις παραγγελίες για έλεγχο υπολοίπου  'βλεπε mdesmia
...
On Error GoTo MHNYMA
   ...
   If Grid1.row > 12 Then 'f_SeiresTimologioy Then
...
If KeyCode = 117 Then   ' --------------  F6 ------ xroma-megetos
...
If KeyCode = 13 Then   'enter ------------------------------------------------
   ...
   GridEidon.Columns(1).width = 2000
...
30   Else ' ************************************************************************
               ' EKTOS KOD ,ONO
         'ΥΠΟΛΟΓΊΖΕΙ ΑΞΊΑ
         If Grid1.Col = f_p Then  '1111111111111111111 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            DINO_POSOTHTA
         End If ' 1111111111111111111111  If Grid1.Col = f_p Then  ' %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         If F_EXO_XROMATA = 1 Then ' ΒΑΖΩ ΤΗΝ ΠΟΣΟΤΗΤΑ ΣΤΟΝ ΠΙΝΑΚΑ ΓΙΑ ΝΑ ΤΟΝ ΤΡΑΒΗΞΕΙ ΣΤΟ ENHM_EGGTIM
                ...
                End If
         End If '  F_EXO_XROMATA = 1 Then ' ΒΑΖΩ ΤΗΝ ΠΟΣΟΤΗΤΑ Σ
...
40      If Grid1.Col <= Grid1.ColS - 3 Then  ' πάει στην επόμενη κολόνα   @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
           ...
           If Grid1.Col = f_f And f_CanChangeFpa = 0 Then 'FPA  //////////////////////////
                 ...
                 If F_SYMPYKNVSH = 1 Then
                    ...
                    Next
                 End If
              'αν δεν βρηκε ξανά τον ίδιο κωδικό ή δεν δέχεται συμπύκνωση
              ...
              End If
              Grid1.Col = 1
           End If  ' --------------If Grid1.Col = f_f And f_CanChangeFpa = 0 Then ////////////////////
           Me.Caption = "125"
        End If '------- If Grid1.Col <= Grid1.ColS - 3 Then  ' πάει στην επόμενη κολόνα @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    '  End If   ' (((((((((((((**************************If Grid1.Col < f_p And (Grid1.Text <> TIMText3.Text Or Val(Grid1.TextMatrix(Grid1.row, f_p)) = 0) Then  ' αλλαξε ο κωδικός άρα ψάξε στο αρχείο
     
End If ' KEYCODE=13 -------------------------------------------------------

 If KeyCode = 116 Then   ' f5 teleytaia timh
     Me.Caption = 30
...
50 If KeyCode = 38 Then   ' ΠΑΝΩ ΒΕΛΟΣ
 ...
 If KeyCode = 120 Then   ' f9 ypologizo τιμη χορισ φπα
     KOMA = 1
 ...
 If KeyCode = 119 Then   ' f8 ypologizo posotita apo ajia
    Dim m_aj, MR
...
' & " WHERE EGGTIM.APOT=" + left(APOTH1.Text, 1) + " AND ERG  LIKE '" + SameLetters(Trim(Mid(TIMText3.Text, 2, 14))) + "%' GROUP BY ONO,KOD,XTI,LTI,LTI5,EID.FPA,MON,POS_KERD,ERG,SUBSTRING(MEMO,1,14) ;"
            ...
            ElseIf Left(TIMText3.Text, 1) = "+" Then ' $$$$$$$
                 ...
                 & " EID.ONO,XTI,LTI,LTI5,EID.FPA,MON,KOD,POS_KERD,ERG,SUBSTRING(MEMO,1,14) AS MMEMO,SYSKMAX,KATHGORIA,CH1 " _
                 & " from EID LEFT JOIN EGGTIM  ON EID.KOD=EGGTIM.KODE" _
                 & " WHERE KODERG = '" + SameLetters(Trim(mID(TIMText3.Text, 2, 14))) + "' GROUP BY ONO,KOD,XTI,LTI,LTI5,EID.FPA,MON,POS_KERD,ERG,SUBSTRING(MEMO,1,14),SYSKMAX,KATHGORIA,CH1 ;"
...
20          Else   ' $$$$$$$
            ...
            EID.Refresh
             ...
             'ψάχνει κατά όνομα
         ElseIf Grid1.Col = f_o Then  '%%%%%%%%%%%%%%%%%If Grid1.Col = f_k Then
            
             GridEidon.Columns(0).width = 5000
             GridEidon.Columns(1).width = 1000
       ...
       intRecord = EID.Recordset.RecordCount
...
Sub DINO_POSOTHTA()
            ...
            End If
             ...
             '-------222222222222222222222222222222222222222222222
              If fbuff = "e" And (F_POS_APOU = 2 Or F_POS_APOU = 3) And f_checkYpol > 0 And F_STADIO <> 2 Then  '22222222222222222222222  F_STADIO<>2 ΔΕΝ ΕΙΝΑΙ ΠΑΡΑΓΓΕΛΙΑ
               ...
               If IsEmpty(F_SHMADEMENA_DELTIA) Or Len(Trim(F_SHMADEMENA_DELTIA)) = 0 Then
                  F_SHMADEMENA_DELTIA = "''"
               End If
               ...
               If f_mono_apoParaggelia Then  '' τιμολογηση μόνο από παραγγελίες
                  HDH_FORTOMENA = 0
                  ...
                  sql2 = "SELECT SUM(POSO) FROM EGGTIM left JOIN TIM ON EGGTIM.ATIM=TIM.ATIM AND EGGTIM.HME=TIM.HME  WHERE" _
                   ...
                   If R(0) = 0 Or IsNull(R(0)) Then
                    ...
                    TIMText3.Text = 0
               End If ' 33333333333333333333333333333333  f_mono_apoParaggelia Then  '' τιμολογηση μόνο από παραγγελίες
               ...
               End If  '22222222222 %%%%%%%%%fbuff = "e" And (F_POS_APOU = 2 Or F_POS_APOU = 3) And f_checkYpol > 0 And F_STADIO <> 2 Then  ' F_STADIO<>2 ΔΕΝ ΕΙΝΑΙ ΠΑΡΑΓΓΕΛΙΑ
                 ...
                 If Val(TIMText3.Text) > (mXre - mPIS) - mDesmia Then
                    MHNYMA2.Timer1.Interval = 0
                    MHNYMA2.Caption = "ΥΠΕΡΒΑΣΗ ΥΠΟΛΟΙΠΟΥ"
                     MHNYMA2.Label1.width = 2 * MHNYMA2.Label1.width
                     
                    MHNYMA2.Label1.Caption = "υπάρχουσα ποσότητα " + Format(mXre - mPIS, "###0") + " ΔΕΣΜ:" + Format(mDesmia, "####")
                    MHNYMA2.Label2.Caption = "" '"υπάρχουσα ποσότητα " + Format(mXre - mPIS, "###0")+ " ΔΕΣΜ:" + Format(mDesmia, "####")
                    MHNYMA2.SHOW 1
                  If f_checkYpol = 2 Then
                     TIMText3.Text = 0
                  End If
                 End If '&&&&&&&&& If Val(TIMText3.Text) > (mXre - mPIS) - mDesmia Then
          End If
...
290           Print #1, Tab(mPcol(K)); GFORMAT(mM, gpic(e + K + 1));
         ...
         'If syn_row >= m_seir_synol + 1 Then
         '   For K = 1 To e
         '       MM = ePict(K)
         '       Printer.Print Format(eField(K), MM)
         '   Next
         '   syn_row = m_syn_row
         '   Printer.Print
         'End If
...
' GoTo 400
       ...
       '*******************  ΣΥΝΟΛΑ *******************
       ' For K = 1 To m_seir_synol - syn_row: Printer.Print: Next 'βρισκω την σειρά  των synolon
